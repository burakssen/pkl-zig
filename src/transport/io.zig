...
pub fn writeLoop(self: *Transport) !void {
    ...
    while (true) {
        const outgoing = self.outgoing.getOne(self.io) catch |err| switch (err) {
            error.Closed => {
        ...
        defer self.allocator.free(outgoing.data);

        log.debug("Writing outgoing frame: {} bytes.", .{outgoing.data.len});
        stdin.writeStreamingAll(self.io, outgoing.data) catch |err| {
            if (isStopping(self)) {
        ...
        log.debug("Wrote outgoing frame: {} bytes.", .{outgoing.data.len});

        if (outgoing.flush_token) |token| {
            self.flushed.putOne(self.io, token) catch |err| {
                if (isStopping(self)) {
                    log.debug("Stopped acknowledging flushed frame during transport shutdown: {}.", .{err});
                } else {
                    log.err("Failed to acknowledge flushed frame: {}.", .{err});
                }
                return err;
            };
        }
